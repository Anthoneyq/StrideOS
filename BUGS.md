...
Verified: **40/40 benchmarks** (31 original + 9 new audit probes), moat backtest re-run on the full shipped path (median 1.2% vs Riegel 1.9%, 210/292 wins), launch smoke ok, JS parses, no dup IDs.
...
- **✅ BUG-036 · Backtest validated a different engine than ships (P2).** `moat_backtest.js` now scores the **full shipped path** (ensemble + the ≥3000m equivalent-performance nudge — personal-k/floor need multi-PR context a single-anchor pair doesn't carry, so this IS the single-PR shipped path) and reports honest n. Full-path result: **median 1.2% vs 1.9%, 72% wins, mid-gap 1.2% vs 2.1%** — claim survives; copy updated to ~33%. *Open question for Anthoney (not a bug): the nudge measured slightly net-negative on the 58 predictions it changes (audit: 1.011%→1.082% median on those) — drop/retune it, or keep for its table-consistency value.*
