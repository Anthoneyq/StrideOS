...
All 📝 OPEN (found, not fixed). Prior fixes BUG-005/006/007/015/020/027 re-verified holding; 31/31 benchmarks green.

- **📝 BUG-028 · Soft short PR anchors mile-family forecasts — no pace ceiling (P1, trust-killer).** `nearestAnchorForTarget` (8401) lets a contradicted (soft) short PR anchor every log-nearer target; the pace floor (5960–5971) excludes contradicted PRs from the *floor* but no symmetric *ceiling* exists. Confirmed: 5K 15:30 + old 800m 2:50 → 1600m forecast **6:09.8 @69% conf** (should be ~4:38 off the 5K; ~92 s wrong), pace inversion within the same table (1600m slower pace than 3000m), forecast slower than the athlete's own 5K. Mirror of BUG-027 (floor exists, ceiling missing).
- **📝 BUG-029 · Mile 1609 falls out of the hybrid ensemble branch (P2).** `_ensembleWeights` (5697) `d2 <= 1600` excludes the Mile → adjacent forecast rows invert: 400m 52.0 anchor → 1600m 4:10.8 but Mile **3:58.7** (12.1 s faster at the longer distance). Fix: `<= 1609` (or 1700 per EVENT_DOMAINS).
- **📝 BUG-030 · No distance/hybrid→sprint weight branch (P2).** 800→400 etc. fall to the flat default (Purdy 0, VDOT included though invalid at sprint durations): 800m 2:00 → 400m 56.1 s vs own-Purdy 53.0 (NFHS ~53.5); 400→800→400 round-trip drifts +1.95 s (3.8%). Benchmarks only test upward.
- **📝 BUG-031 · Event Fit "median" = upper-middle prediction (P2).** buildEventFit (9225) takes `arr[floor(n/2)]` — the *slower* middle for even counts (any 3- or 5-PR athlete) → every diffPct inflated. Demo athlete Hannah K.: 1600m fit flips **BALANCED→STRONG** (+1.45%→+4.10%). Hits Event Fit card, free Event Finder, Multi-Event. Correct `_med` (6513) unused. One-line fix.
- **📝 BUG-032 · Training-group median = upper-middle member (P2).** computeTrainingGroups (7023) same bug: every even-sized group anchors rep targets to its slower middle member (2-person group 18:00/20:00 → all targets from 20:00, ~5% too slow; free-tier 6-athlete roster = 3 pairs → every group). Flows to groupSessionZones/renderSquads/printGroupSheets. One-line `_med` fix.
- **📝 BUG-033 · Two adjacent "1km" columns in the Coach Split Table (P2).** `repLabel(1200)` → "(1.2).toFixed(0)+'km'" = "1km" (4956), colliding with the real 1km rep on Tempo/Threshold/CV/Race/VO2 rows. Print cards unaffected.
- **📝 BUG-034 · UTC/local date mixing (P2).** (a) prFreshness (3824) parses race dates UTC-midnight vs local now → "1 days old" same-evening, 90/365-day boundaries flip ~6–7 pm CT, east-of-UTC today-dated races read "future/INVALID" (+2.0× penalty). (b) Workout default date (2142) = `toISOString()` → **tomorrow** every US evening; can bucket a run into next week.
- **📝 BUG-035 · Public claim "290+ real runners" is wrong (P1, copy).** index.html:8024 — reality: **292 ordered pairs from 23 runners** (79 seasons); the 292 also double-counts both directions (r=0.941 between directions) → effective n≈146 unique pairs. Verdict survives (p<1e-6 vs Riegel) but the shipped copy inflates athletes ~12.7×, and the "widens at ≥4× distance" claim rests on 10 unique pairs / 6 elite athletes.
- **📝 BUG-036 · Backtest validates a different engine than ships (P2).** moat_backtest scores bare `strideEnsemble`; shipped `raceForecastForTarget` adds the 30% OBSERVED_RATIOS nudge + personal-k + floor. Replaying 292 pairs with the nudge: 58 predictions change, median |%err| on those **worsens 1.011%→1.082%**. Decide: backtest full path or drop/retune the nudge.
- **📝 BUG-037 · Latent reciprocal-model trap under dead VDOT zone path (P3).** vdotPctForZone (3893) inverts with the reciprocal model BUG-005 removed from pctToMult; masked only by BUG-014's units bug keeping the path dead. Fixing the units alone ships Easy ~29 s/mi too fast. Fix both together or neither.
- **Minor (no numbers):** fmtT floor-vs-round asymmetry >1 h; `fmt400` "60.0/400" edge; `parseDurationToSec` accepts "1:75"; `_normDate` east-of-UTC day shift; additional PRs hard-coded `raceDate:null` (always freshness-unknown); `diagnosis.py` = legacy + internally broken, never cite (details in the audit file).
