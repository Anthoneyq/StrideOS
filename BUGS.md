...
Verified: **59/59 benchmarks** (31 original + 28 new audit probes covering BUG-028..034, BUG-037 and every executable minor), moat backtest re-run in BOTH modes (Mode A single-anchor shipped path: median 1.2% vs Riegel 1.9%, 210/292 wins · Mode B multi-PR leave-one-out through `raceForecastForTarget`: 1.4% vs 1.6% against an anchor-matched Riegel, 123/187 wins), launch smoke ok, JS parses, no dup IDs. Codex review cycle 1 CHANGES-REQUESTED ×3 → all addressed (Mode B implemented rather than narrowing the claim; "races" → "predictions/event pairs" units everywhere public; the missing regression probes added).
...
- **✅ BUG-035 · "290+ real runners" copy (P1).** Home page now says "292 held-out races from 23 real athletes' careers … ~33% closer"; MOAT_EVIDENCE restates the honest n (~146 unique pairs, 23 athletes, far-band = 10 unique pairs — quote mid-gap publicly); SUPERIORITY + FOUNDING-COACH-OUTREACH updated to match.
- **✅ BUG-036 · Backtest validated a different engine than ships (P2).** `moat_backtest.js` now runs TWO modes: **Mode A** — the single-anchor shipped path (ensemble + the ≥3000m equivalent-performance nudge; what a single-PR athlete's coach sees): median **1.2% vs 1.9%**, 72% wins, mid-gap 1.2% vs 2.1%. **Mode B** (added after codex review) — true multi-PR leave-one-out through `raceForecastForTarget` itself (nearest-anchor + personal-k + floor/ceiling — the in-app Proof Ledger mechanism): **1.4% vs 1.6%** against Riegel *given the same nearest anchor*, 123/187 wins. Copy updated to ~33% (Mode A vs the commodity formula). *Open question for Anthoney (not a bug): the nudge measured slightly net-negative on the 58 predictions it changes (audit: 1.011%→1.082% median on those) — drop/retune it, or keep for its table-consistency value.*
