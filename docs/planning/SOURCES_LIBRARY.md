...
STRIDE OS displays a short proprietary statement and links here for coaches who want methodological depth. This is deliberate:

1. Coaches in the field don't need to see 40+ academic citations every time they open the app
2. Showing specific formula names (Riegel, Cameron, VDOT, etc.) introduces cognitive bias — coaches may dismiss a prediction because they personally dislike one of those formulas, even if the ensemble produces better accuracy than any single formula alone
...
Beyond the peer-reviewed sources cited below, STRIDE OS maintains validation corpora of competitive race results used to empirically calibrate ensemble weights, benchmark population-level performance distributions, and validate confidence intervals. These datasets supplement the published literature; they do not replace it.
...
**1,817 race records spanning ages 12–95** across three populations (Texas HS, NCAA D1, Masters). Used to validate formula accuracy, benchmark performance distributions, and calibrate confidence intervals.

**Data file:** `/data/validation_corpus.csv` in this repository.
...
**n = 2,303 runners.** Empirical validation showing all formulas — including ensembles — overestimate marathon time for many recreational runners.
...
The lactate-controlled, double-threshold approach pioneered by Marius Bakken (Norway, early 2000s) has become the dominant training methodology in NCAA Division I distance and elite professional running over the past decade. STRIDE OS's Tempo (~87%) and Threshold (~92%) zones map directly to this framework.
...
Practical application of the lactate-controlled Norwegian threshold model. Defines LT1 (~2.0 mmol/L, AM cruise/tempo) and LT2 (~3.0–3.5 mmol/L, PM threshold). Double-threshold days as the central training stimulus.
...
MAF heart rate (180 minus age) for easy aerobic running. Underpins the "by feel" approach to easy-pace prescription. For most trained athletes this lands at ~65–75% of race pace — the Easy zone in STRIDE OS.
...
Elite-coaching methodology prescribing training intensities as percentages of race PR. Widely used at world-class level.

**Davis, J. J.** (2023). *A comprehensive overview of Canova-style "full-spectrum" percentage-based training.* runningwritings.com.
Mathematical treatment of Canova's percentage system.
...
- **Marathon predictions from 5K/10K still carry meaningful uncertainty** — published validation shows even ensemble methods overestimate marathon time for some recreational runners. *Source: Vickers & Vertosick (2016).*
